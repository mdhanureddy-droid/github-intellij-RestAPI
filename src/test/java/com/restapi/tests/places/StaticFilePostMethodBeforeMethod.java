...
@Log4j2
public class StaticFilePostMethodBeforeMethod {
    ...
    public void beforeMethod(ITestResult result) throws IOException {
        ...
        if ("updatePlace".equals(methodName) && placeId == null) {
            addPlace();
        } else if ("verifyUpdatedAddress".equals(methodName)) {
            if (placeId == null) addPlace();
            if (updatedAddress == null) updatePlace();
        }
    }
